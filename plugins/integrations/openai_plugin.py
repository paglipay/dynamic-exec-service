...
class OpenAIFunctionCallingPlugin:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "\n  0. FILE SHARED WITH NO EXPLICIT REQUEST: if the user uploaded or shared a file without asking "
            "for analysis, EXIF data, or a description — just acknowledge receipt (e.g. 'Got it! Let me know what you need.'). "
            "Do NOT describe the image. Do NOT call read_image_for_vision. Do NOT call read_image_gps. "
            "Wait for the user to ask something specific."
            ...
            "\n  2. VISUAL ANALYSIS REQUESTS (describe, what is shown, analyze, identify, what do you see, etc.): "
            "if image pixels are already in this conversation as vision content, describe them directly. "
            "If pixels are NOT in context, call FileReaderPlugin.read_image_for_vision with the saved file path, "
            "then describe the result."
            "\n  3. ALL OTHER REQUESTS (file info, storage, questions unrelated to image content): "
            "do not load or analyze image pixels at all. Use the saved file path and metadata only. "
            "Never proactively describe an image or load it for vision unless the user explicitly asks."
            "\nPixels are only sent in context when the user has explicitly requested visual analysis. "
            "For everything else, image files are available on disk via their saved path. "
